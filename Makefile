...
# Deployment helpers
deploy-local :; FOUNDRY_PROFILE=production forge script script/Compota.s.sol --rpc-url localhost --broadcast -v
deploy-sepolia :; FOUNDRY_PROFILE=production forge script script/Compota.s.sol --rpc-url sepolia --broadcast -vvv
...
coverage:
	FOUNDRY_PROFILE=$(profile) forge coverage -no-match-test testFuzzCalculateStakingRewards --no-match-path 'test/invariant/**/*.sol' --report lcov && lcov --extract lcov.info -o lcov.info 'src/*' && genhtml lcov.info -o coverage
